...
function onStatus(status) {
  if (status === Strophe.Status.CONNECTING) console.log("• connecting");
  else if (status === Strophe.Status.AUTHFAIL) { console.error("‼ auth failed — check BOT_PASS"); process.exit(1); }
    ...
    console.error(`• disconnected — reconnecting in ${Math.round(delay / 1000)}s`);
    setTimeout(() => conn.connect(`${USER}@${DOMAIN}/assistant`, PASS, onStatus), delay);
  }
}
