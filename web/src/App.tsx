...
import { getNotif, pushMessage, pushOnline, requestPush, enableWebPush } from "./notify";
...
export function App() {
  ...
  function doConnect(username: string, password: string, server: { ws: string; domain: string } | undefined, isReconnect: boolean) {
    ...
    client.on("message", (msg) => {
      ...
      if (added && !msg.outgoing && !msg.delayed) {
        ...
        pushMessage({ title: nameOf(msg.from), body: replyLabel(msg), icon: getPhoto(msg.from), onClick: () => openChatNotif(msg.from) });
      }
    });
    ...
    client.on("groupMessage", (room, msg) => {
      ...
      if (added && !msg.outgoing && !msg.delayed) {
        if (getNotif().sound) playMessage();
        pushMessage({ title: `# ${room.split("@")[0]}`, body: `${msg.from}: ${replyLabel(msg)}`, onClick: () => openChatNotif(room) });
      }
    });
    ...
    client.on("secretInit", async (peer, { k, idk, sig }) => {
      try {
        ...
        if (secretPriv.current.has(peer)) {
          if (selfJidRef.current < peer) return;
          secretPriv.current.delete(peer); secretMyPub.current.delete(peer);
        }
        if (!(await e2e.verifyPub(idk, k, sig))) { console.warn("secretInit: bad identity signature from", peer); return; }
        ...
        client.sendSecretAck(peer, myPub, id.pub, await e2e.signPub(id.priv, myPub));
    ...
    client.on("secretAck", async (peer, { k, idk, sig }) => {
      try {
        ...
        if (!myPriv || !id) return;
        if (!(await e2e.verifyPub(idk, k, sig))) { console.warn("secretAck: bad identity signature from", peer); return; }
    ...
    client.on("secretMsg", async (peer, { id, iv, ct, ttl }) => {
      ...
      try {
        ...
        if (attachment && attachment.fileIv && attachment.url) {
          if ((attachment.size || 0) > 25 * 1024 * 1024) { attachment = undefined; }
          else try {
            const encBuf = await (await fetch(attachment.url)).arrayBuffer();
  ...
  function verifySecret(convJid: string) {
  ...
  async function startSecretChat(userOrJid: string): Promise<string> {
    ...
    try {
      ...
      client.sendSecretInit(peer, myPub, id.pub, await e2e.signPub(id.priv, myPub));
  ...
  async function loadOlder(jid: string): Promise<boolean> {
    ...
    try {
      ...
      mamCursor.current[jid] = { first: next.first ?? cur.first, complete: next.complete };
