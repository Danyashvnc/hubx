...
export async function signPub(
  ...
  const sig = await crypto.subtle.sign(ECDSA_SIGN, idPriv, enc.encode(sigPayload(ctx, fromJid, toJid, ephemeralPub)) as BufferSource);
  return toB64(new Uint8Array(sig));
}

export async function verifyPub(
  idPubJwk: JsonWebKey,
  ephemeralPub: JsonWebKey,
  sigB64: string,
  ctx: string,
  ...
  try {
    ...
    return await crypto.subtle.verify(ECDSA_SIGN, idPub, fromB64(sigB64) as BufferSource, enc.encode(sigPayload(ctx, fromJid, toJid, ephemeralPub)) as BufferSource);
  } catch { return false; }
}
