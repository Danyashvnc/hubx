...
export function ChatThread({
  ...
  useEffect(() => () => window.clearTimeout(composeTimer.current), []);
  useEffect(() => {
  ...
  useEffect(() => { setMenuFor(null); setMenuPos(null); setEditing(null); setReplying(null); setDragOver(false); setPending(null); setHighlight(null); setTtlMenu(false); setEmojiOpen(false); setSelectMode(false); setSelected(new Set()); setDelModalOpen(false); setHistDone(false); setLoadingOlder(false); prependingRef.current = false; }, [contact.jid]);
  function onDraft(v: string) {
    ...
    composeTimer.current = window.setTimeout(() => client.sendComposing(contact.jid, false), 2500);
  }
