...
export function AdminPanel({ selfJid, onOpenChat }: { selfJid: string; onOpenChat: (jid: string) => void }) {
  ...
  const [invites, setInvites] = useState<{ code: string; uses: number; maxUses: number; exp: number; createdAt: number }[]>([]);
  ...
  const loadInvites = useCallback(() => { api.listInvites().then(setInvites).catch(() => {}); }, []);
  useEffect(() => { loadInvites(); }, [loadInvites]);
  async function genInvite() {
    setInviteErr(undefined);
    const code = await api.createInvite(1, 7);
    if (code) { setNewCode(code); loadInvites(); } else setInviteErr("Не удалось создать код");
  }
  async function revokeInvite(code: string) { await api.revokeInvite(code); if (newCode === code) setNewCode(undefined); loadInvites(); }
  ...
  return (
    <div className="admin">
      ...
      <div className="hp" style={{ maxWidth: 620, marginBottom: 16 }}>
        <div className="hp-title row"><span>Коды‑приглашения</span>
          <button className="mini-btn" onClick={genInvite}>+ Создать код</button>
        </div>
        {newCode && <div className="info-row"><span>Новый код (передайте пользователю)</span><b className="ip-mono">{newCode}</b></div>}
        {inviteErr && <div className="info-empty">{inviteErr}</div>}
        {invites.length === 0 && !newCode && <div className="info-empty">Кодов нет. Регистрация закрыта: создайте код и передайте его новому пользователю.</div>}
        {invites.map((iv) => (
          <div className="info-row" key={iv.code}>
            <b className="ip-mono">{iv.code}</b>
            <span>{iv.uses}/{iv.maxUses} · до {new Date(iv.exp).toLocaleDateString()} <button className="link-btn danger" onClick={() => revokeInvite(iv.code)}>отозвать</button></span>
          </div>
        ))}
      </div>
